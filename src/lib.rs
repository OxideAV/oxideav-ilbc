...
/// Register the iLBC codec with the supplied [`CodecRegistry`]. Prefer
/// the unified [`register`] entry point when you have a
/// [`oxideav_core::RuntimeContext`] in hand.
pub fn register_codecs(reg: &mut CodecRegistry) {
    codec::register_codecs(reg);
}

/// Unified registration entry point — installs iLBC into the codec
/// sub-registry of the supplied [`oxideav_core::RuntimeContext`].
pub fn register(ctx: &mut oxideav_core::RuntimeContext) {
    register_codecs(&mut ctx.codecs);
}

oxideav_core::register!("ilbc", register);
